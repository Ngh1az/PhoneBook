...
class GroupView(tk.Frame):
    def __init__(self, parent, user_data, on_logout, on_switch_view):
        ...
        self.controller = GroupController(user_data["user_id"])
    ...
    def create_widgets(self):
        ...
        columns = ("Tên nhóm", "Mô tả", "Số liên hệ")
        ...
        self.tree.column("Tên nhóm", width=200)
        ...
        tk.Button(
            action_frame,
            text="Sửa",
            command=self.show_edit_group_dialog,
            bg=COLORS["warning"],
            fg=COLORS["white"],
            font=FONTS["button"],
            width=10,
        ...
        tk.Button(
            ...
            command=self.delete_multiple_groups,
    ...
    def show_add_group_dialog(self):
        ...
        tk.Label(
            form_frame, text="Tên nhóm:", font=FONTS["normal"], bg=COLORS["bg"]
        ...
        def add_group():
            ...
            if not name:
                show_error("Lỗi", "Vui lòng nhập tên nhóm")
                return

            success, message = self.controller.add_group(
    ...
    def show_edit_group_dialog(self):
        ...
        if not selected:
            show_error("Lỗi", "Vui lòng chọn nhóm cần sửa")
            return
        ...
        def update_group():
            ...
            success, message = self.controller.update_group(
                group_id, name, description if description else None
            )
    ...
    def delete_group(self):
        ...
        selected = self.tree.selection()
        if not selected:
            show_error("Lỗi", "Vui lòng chọn nhóm cần xóa")
            return
        ...
        if ask_yes_no("Xác nhận", f"Bạn có chắc muốn xóa nhóm '{group_name}'?"):
            success, message = self.controller.delete_group(group_id)
    ...
    def delete_multiple_groups(self):
        """Delete multiple selected groups at once"""
        selected = self.tree.selection()
        if not selected:
            show_error("Lỗi", "Vui lòng chọn ít nhất một nhóm cần xóa")
            return

        count = len(selected)
        if not ask_yes_no("Xác nhận", f"Bạn có chắc muốn xóa {count} nhóm đã chọn?"):
            return
        ...
        for item in selected:
            group_id = self.tree.item(item)["tags"][0]
            success, _ = self.controller.delete_group(group_id)
            if success:
                success_count += 1
            else:
                failed_count += 1
        ...
        # Show result
        if failed_count == 0:
            show_success(f"Đã xóa thành công {success_count} nhóm!")
        else:
